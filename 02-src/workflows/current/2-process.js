...
if (prep.kind !== 'pdf' && prep.kind !== 'image' && prep.kind !== 'word') {
  const message = 'I can read a PDF, a Word document, a PNG, or a JPEG. This file is none of those, so I stopped before extraction.';
  return pack({
    route: 'ask',
    message,
    reply_json: JSON.stringify({ message, status: 'needs_details', document_id: '' }),
...
return pack({ route: prep.kind === 'word' ? 'pdf' : prep.kind, message: '', reply_json: '' });
`;
...
const promptIntro = 'You extract fields from one document. Return one JSON object and nothing else. Do not invent a value. If a field is not on the page, use an empty string. Do not choose between two different totals, dates, or party names. Put each conflict in conflicts. Pages may be marked --- PAGE n ---. If a page number is missing or that page has no words, list it in unreadable_pages. type_guess must be invoice, contract, brief, or other. JSON keys: title, parties, counterparty, document_date, effective_date, expiry_date, currency, total_amount, reference_number, why_it_exists, type_guess, unreadable_pages, conflicts. conflicts items look like {"field":"total_amount","values":["100.00","180.00"]}. Dates as YYYY-MM-DD when the document states one. Amounts as digits and a decimal point, no currency symbol. On an invoice, counterparty is the supplier, not the customer.';
const llm_body = {
  storage_key: prep.storage_key || prep.storage_uri || '',
  stated_type: prep.stated_type || '',
  prepared_pages: reportedPages,
...
const pageText = res.page_text || (built && built.page_text) || '';
const reportedPages = res.reported_pages || (built && built.reported_pages) || 0;
return [{ json: Object.assign({}, built || {}, { model_raw: content || '', model_error: modelError, page_text: pageText, reported_pages: reportedPages }) }];
`;
...
const indexes = [];
const re = /---\\s*(?:PAGE|SECTION)\\s+(\\d+)\\s*---/gi;
let match;
...
if (indexes.length) {
  ...
  for (let p = 1; p <= maxPage; p++) {
    let pos = -1;
    for (let i = 0; i < indexes.length; i++) if (indexes[i].page === p) pos = i;
...
function appearsOn(value) {
  const raw = clean(value);
  if (!raw) return 0;
  const forms = [raw.replace(/\s/g, '').toLowerCase()];
  if (raw.indexOf('.') !== -1) forms.push(raw.replace(/\.00$/, '').replace(/\s/g, '').toLowerCase());
  const pages = indexes.length ? indexes : [{ page: 1, index: 0, len: 0 }];
  for (let p = 0; p < pages.length; p++) {
    const start = indexes.length ? pages[p].index + pages[p].len : 0;
    const end = indexes.length && p + 1 < pages.length ? pages[p + 1].index : text.length;
    const slice = text.slice(start, end).replace(/\s/g, '').toLowerCase();
    for (let i = 0; i < forms.length; i++) {
      if (forms[i] && slice.indexOf(forms[i]) !== -1) return pages[p].page;
    }
  }
  return 0;
}
...
if (text.replace(/\s/g, '').length >= 20 && !couldNotSplit) {
  for (const key of Object.keys(materialLabels)) {
    ...
    const page = appearsOn(fields[key]);
    if (!page) {
      if (requiredKeys[key]) problems.push(materialLabels[key] + ' was not on a page I could read, so I did not keep it.');
      fields[key] = '';
    } else if (requiredKeys[key]) {
      evidence.push(materialLabels[key] + ' ' + fields[key] + ' is in ' + (text.indexOf('--- SECTION') === -1 ? 'page ' : 'section ') + page + '.');
    }
  }
}
...
const pageNotes = [
  couldNotSplit
    ...
    : missing.map(p => (text.indexOf('--- SECTION') === -1 ? 'Page ' : 'Section ') + p + ' could not be read because that part has no text. I did not guess what was on it.').join(' '),
  evidence.join(' '),
].filter(Boolean).join(' ');
...
return [{
  json: {
    ...
    field_evidence: JSON.stringify((model && model.field_evidence) || []),
    reviewed_by: '',
    reviewed_at: '',
...
const loadRecords = node({
  ...
  config: {
    ...
    parameters: {
      ...
      query: "WITH claim AS (UPDATE documents SET status = 'processing' WHERE document_id = $1 AND status IN ('received', 'processing')) SELECT * FROM documents WHERE status = 'accepted' AND superseded_by = ''",
      options: { queryReplacement: expr('{{ $json.document_id }}') },
    },
...
const httpImage = node({
  ...
  config: {
    ...
    credentials: { httpBearerAuth: newCredential('OpenRouter', 'OPENROUTER_CREDENTIAL_ID') },
    output: [{ json: { choices: [{ message: { content: '{}' } }] } }],
  },
});
...
const processHook = trigger({
  type: 'n8n-nodes-base.executeWorkflowTrigger',
  version: 1.1,
  config: { name: 'Prepare submission', parameters: { inputSource: 'passthrough' } },
});
